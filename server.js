...
// Tests and unusual installations may override this; normally follow the current user's Pi home.
const DATA_ROOT = process.env.WORKFLOW_DATA_ROOT || path.join(os.homedir(), ".pi", "workflows", "projects");
...
function agentView(a) {
  ...
  const model = text(a.model, 100);
  const requestedModel = text(a.requestedModel, 100);
  const requestedTier = text(a.requestedTier, 80);
  const modelFallback = a.modelFallback === true;
  const modelState = model ? "resolved" : modelFallback ? "fallback" : (requestedModel || requestedTier) ? "resolving" : "unknown";
  const modelLabel = model ? model : modelFallback ? "Session-default model not reported" : modelState === "resolving" ? "Resolving model…" : "Not recorded";
  return { id: text(a.id == null ? a.label || "unknown" : String(a.id), 80), label: text(a.label, 120), status: text(a.status || "unknown", 40), phase: text(a.phase, 100), prompt: text(a.prompt, 280), model, modelState, modelLabel, requestedModel, requestedTier, startedAt: text(a.startedAt, 60), endedAt: text(a.endedAt, 60), tokens: number(a.tokens) || number(usage.total), tokenUsage: { input:number(usage.input), output:number(usage.output), cacheRead:number(usage.cacheRead), cacheWrite:number(usage.cacheWrite), total:number(usage.total) || number(a.tokens) } };
}
...
function runView(project, file) {
  ...
  const agents = Array.isArray(value.agents) ? value.agents.map(agentView) : [];
  return { project, runId:artifactRunId, workflowName:text(value.workflowName, 160), status:text(value.status || "unknown", 40), phase:text(value.currentPhase || phases.at(-1)?.name || "", 120), phases, startedAt:text(value.startedAt, 60), updatedAt:text(value.updatedAt, 60), endedAt:text(value.endedAt, 60), tokenBudget:value.tokenBudget == null ? null : number(value.tokenBudget), tokenUsage:{input:number(usage.input),output:number(usage.output),cacheRead:number(usage.cacheRead),cacheWrite:number(usage.cacheWrite),total:number(usage.total)}, agents, models:[...new Set(agents.filter(agent => agent.modelState === "resolved").map(agent => agent.model))] };
}
function snapshot() {
  const runs = [], skipped = [];
  let projects;
  try { projects = fs.readdirSync(DATA_ROOT, { withFileTypes:true }); } catch { return { projects:[], skipped:["Workflow data directory unavailable"], dataRoot:DATA_ROOT }; }
  ...
  return { projects:[...grouped].map(([name,runs]) => ({name,runs})), skipped, dataRoot:DATA_ROOT };
}
...
function modelChips(models){return (models?.length?models:["No resolved models yet"]).map(model=>'<code class="model-chip">'+esc(model)+'</code>').join('')}
function agentModel(a){const request=a.requestedTier?('Requested tier: '+a.requestedTier):(a.requestedModel?('Requested model: '+a.requestedModel):'');const state=a.modelState==='resolving'?' · '+request:a.modelState==='fallback'?' · requested model unavailable':'';return '<code class="model-chip agent-model">'+esc(a.modelLabel)+'</code>'+(state?'<span class="meta">'+esc(state)+'</span>':'')}
function render(data){document.querySelector("#refresh").textContent="Last refresh: "+new Date(data.refreshedAt).toLocaleTimeString()+" · every 2s";const app=document.querySelector("#app");if(!data.projects.length){const detail=data.skipped?.includes("Workflow data directory unavailable")?'Workflow directory unavailable: ':'No readable canonical run files found in ';app.innerHTML='<div class="empty">'+esc(detail)+'<code>'+esc(data.dataRoot||'the configured workflow directory')+'</code><br><span class="meta">Run a dynamic workflow as this OS user, or set <code>WORKFLOW_DATA_ROOT</code> before starting the monitor.</span></div>';return}app.innerHTML=data.projects.map(p=>'<section class="project"><h2>'+esc(p.name)+' <span class="meta">('+p.runs.length+' runs)</span></h2>'+p.runs.map(r=>'<article class="run"><div class="top"><div><div class="name">'+esc(r.workflowName||r.runId)+'</div><div class="meta"><code>'+esc(r.runId)+'</code></div></div><span class="pill '+cls(r.status)+'">'+esc(r.status)+'</span></div>'+stepper(r.phases)+'<div class="model-summary"><strong>Agent models</strong>'+modelChips(r.models)+'</div><div class="details"><span>Phase<b>'+esc(r.phase||"—")+'</b></span><span>Started<b>'+fmt(r.startedAt)+'</b></span><span>Updated<b>'+fmt(r.updatedAt)+'</b></span><span>Ended<b>'+fmt(r.endedAt)+'</b></span><span>Tokens<b>'+usage(r.tokenUsage)+'</b></span></div>'+buttons(r)+'<div class="agents">'+(r.agents.length?r.agents.map(a=>'<button class="agent agent-button" data-agent-id="'+esc(a.id)+'" data-project="'+esc(r.project)+'" data-run="'+esc(r.runId)+'"><div class="agenthead"><b>Agent '+esc(a.id)+(a.label?' · '+esc(a.label):'')+'</b><span class="pill '+cls(a.status)+'">'+esc(a.status)+'</span>'+agentModel(a)+'<span class="meta">'+esc(a.phase||"")+'</span><span class="hint">View live activity →</span></div><div class="meta">'+fmt(a.startedAt)+' → '+fmt(a.endedAt)+' · '+usage(a.tokenUsage)+'</div>'+(a.prompt?'<div class="prompt">'+esc(a.prompt)+'</div>':'')+'</button>').join(''):'<div class="meta">No agents recorded yet.</div>')+'</div></article>').join('')+'</section>').join('')}
let selectedAgent=null;
function renderAgentDetail(data){const a=data.agent;const modelHeading=a.modelState==='resolved'?'Model used by this agent':a.modelState==='resolving'?'Model resolution in progress':a.modelState==='fallback'?'Model fallback':'Model information';const modelNote=a.modelState==='resolving'?(a.requestedTier?'Requested tier: '+a.requestedTier:a.requestedModel?'Requested model: '+a.requestedModel:'Waiting for the workflow runtime to report the resolved model.') :a.modelState==='fallback'?'The requested model was unavailable; the workflow runtime did not report its session-default model.':a.modelState==='unknown'?'The workflow runtime did not record a resolved model.':'';document.querySelector("#agent-title").innerHTML='Agent '+esc(a.id)+(a.label?' · '+esc(a.label):'')+' <span class="pill '+cls(a.status)+'">'+esc(a.status)+'</span>';document.querySelector("#agent-subtitle").textContent=(data.run.workflowName||data.run.runId)+' · '+(a.phase||data.run.phase||'');const events=a.history||[];document.querySelector("#agent-detail").innerHTML='<div class="model-feature"><span>'+esc(modelHeading)+'</span><code>'+esc(a.modelLabel)+'</code>'+(modelNote?'<div class="meta" style="margin-top:7px">'+esc(modelNote)+'</div>':'')+'</div><div class="details"><span>Started<b>'+fmt(a.startedAt)+'</b></span><span>Ended<b>'+fmt(a.endedAt)+'</b></span><span>Tokens<b>'+usage(a.tokenUsage)+'</b></span></div>'+(a.prompt?'<h3>Mission</h3><div class="fullprompt">'+esc(a.prompt)+'</div>':'')+(a.resultPreview?'<h3>Latest result</h3><div class="fullprompt">'+esc(a.resultPreview)+'</div>':'')+'<h3>Live activity <span class="meta">('+events.length+' latest events)</span></h3><div class="timeline">'+(events.length?events.map(x=>'<article class="event '+(x.isError?'error':'')+'"><div class="eventmeta">'+esc(x.role||'event')+(x.kind?' · '+esc(x.kind):'')+(x.toolName?' · '+esc(x.toolName):'')+(x.timestamp?' · '+new Date(x.timestamp).toLocaleTimeString():'')+'</div><pre>'+esc(x.text||'—')+'</pre></article>').join(''):'<div class="empty">No detailed activity recorded yet.</div>')+'</div>'}
